...
export default function PlanEditPage() {
    ...
    adminService.getPlan(String(id))
      .then((p) => {
        setForm({
          ...
          frequency: p.frequency, is_default: p.is_default,
          trial_days: p.trial_days, no_of_vcards: p.no_of_vcards, status: p.status,
        })
        if (p.plan_feature) {
          const feat: Record<string, boolean> = {}
          allFeatureKeys.forEach((k) => { feat[k] = !!(p.plan_feature as any)?.[k] })
          setFeatures(feat)
        }
      })
  ...
  return (
    <div>
      <Helmet><title>{t('admin.edit_plan')} - Mobile VCard Link</title></Helmet>
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <Link to={`/admin/plans/${id}`} className="hover:text-orange-600 transition-colors">{form.name}</Link>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
            <h1 className="text-xl font-bold text-gray-900 dark:text-white mb-6">{t('admin.edit_plan')}: {form.name}</h1>
