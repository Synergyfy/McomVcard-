...
export default function TemplateCustomizePage() {
  ...
  const [website, setWebsite] = useState('greenleaf.com')
  ...
  return (
    <div>
      <Helmet><title>Customize Template - MCOM VCard Social Bio</title></Helmet>

      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-1">
            <Link to="/user/templates" className="hover:text-orange-600">Templates</Link>
            <span>/</span>
            <span className="text-gray-900 dark:text-white">{template.name}</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Customize Template</h1>
        </div>
        <div className="flex gap-2">
          <Link to="/user/templates" className="px-4 py-2 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
            Cancel
          </Link>
          <Link to="/user/vcards/create" className="px-4 py-2 bg-orange-500 text-white text-sm font-semibold rounded-lg hover:bg-orange-600 transition-colors shadow-sm shadow-orange-200 dark:shadow-none">
            Use This Template
          </Link>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {tab === 'content' && (
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5 space-y-4">
              <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Business Info</h3>
              <div>
            ...
            <div className="w-[320px] rounded-[2.5rem] border-[6px] border-gray-900 dark:border-gray-700 shadow-2xl overflow-hidden bg-white">
              ...
              <div className="min-h-[540px] overflow-hidden" style={{ fontFamily: fontFamily }}>
                ...
                <div className={`px-5 -mt-12 ${logoPosition === 'center' ? 'text-center' : logoPosition === 'right' ? 'text-right' : ''}`}>
                  <div className={`w-20 h-20 rounded-full border-4 border-white shadow-lg mx-${logoPosition === 'center' ? 'auto' : logoPosition === 'right' ? 'ml-auto' : ''} flex items-center justify-center text-white text-xl font-bold`} style={{ backgroundColor: primaryColor }}>
                    {businessName.charAt(0)}
                  </div>
                  <h2 className="mt-3 text-lg font-bold text-gray-900">{businessName}</h2>
