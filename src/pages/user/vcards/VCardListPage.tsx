...
type MainTab = 'vcards' | 'my-templates' | 'browse'
type BrowseSubTab = 'claimed' | 'all'

export default function VCardListPage() {
  const [mainTab, setMainTab] = useState<MainTab>('vcards')
  const [browseSubTab, setBrowseSubTab] = useState<BrowseSubTab>('claimed')
  ...
  const allPublished = mockTemplates.filter((t) => t.status === 'published' && !claimedIds.includes(t.id))
  ...
  const displayList = browseSubTab === 'claimed' ? filterTemplates(claimedTemplates) : filterTemplates(allPublished)
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6 border-b border-gray-200 dark:border-gray-700 pb-0">
        {([
          ...
          { key: 'my-templates' as const, label: 'My Templates', count: claimedTemplates.length },
          { key: 'browse' as const, label: 'Browse Templates', count: allPublished.length },
        ]).map((t) => (
      ...
      {/* ════════════════════════════════════════════════════════════════
          TAB 2: My Templates (claimed)
         ════════════════════════════════════════════════════════════════ */}
      {mainTab === 'my-templates' && (
        claimedTemplates.length ? (
                ...
                <div className="relative aspect-[3/4] overflow-hidden bg-gray-100 dark:bg-gray-700">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="flex gap-2">
                      <Link to={`/user/vcards/create?template=${t.id}`} className="flex-1 text-center py-1.5 bg-white rounded-lg text-xs font-semibold text-gray-900 hover:bg-gray-100">Use Template</Link>
                      <Link to={`/user/templates/${t.id}/customize`} className="flex-1 text-center py-1.5 bg-orange-500 rounded-lg text-xs font-semibold text-white hover:bg-orange-600">Customize</Link>
                    </div>
                  </div>
                </div>
                <div className="p-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-medium text-gray-900 dark:text-white">{t.category}</span>
                    {t.customized ? (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 font-medium">Customized</span>
                    ) : (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400 font-medium">Default</span>
                    )}
                  </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-12 text-center">
            ...
            <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">No templates claimed yet</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">Browse and claim a template to get started</p>
            <button onClick={() => setMainTab('browse')} className="inline-block px-4 py-2 bg-orange-500 text-white text-sm font-semibold rounded-lg hover:bg-orange-600 transition-colors">
              Browse Templates
            </button>
          </div>
        )
      ...
      {/* ════════════════════════════════════════════════════════════════
          TAB 3: Browse Templates (merged from /user/templates)
         ════════════════════════════════════════════════════════════════ */}
      {mainTab === 'browse' && (
        <>
          {/* Sub-tabs */}
          <div className="flex gap-2 mb-4">
            <button onClick={() => setBrowseSubTab('claimed')}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${browseSubTab === 'claimed' ? 'bg-orange-500 text-white shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
              My Claimed ({claimedTemplates.length})
            </button>
            <button onClick={() => setBrowseSubTab('all')}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${browseSubTab === 'all' ? 'bg-orange-500 text-white shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
              Available ({allPublished.length})
            </button>
          </div>
          ...
          {displayList.length ? (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-12 text-center">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">{browseSubTab === 'claimed' ? 'Claim a template to get started' : 'Try adjusting your filters'}</p>
            </div>
          )}
        </>
      )}
