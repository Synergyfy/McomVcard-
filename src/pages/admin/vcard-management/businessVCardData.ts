...
export interface BusinessVCard {
  id: number; preview: string; businessName: string; businessId: string; membership: string
...
export function businessesForTemplate(templateId: number): BusinessVCard[] {
  ...
  return ids.map(id => BUSINESS_VCARDS.find(v => v.id === id)).filter((v): v is BusinessVCard => Boolean(v))
}
