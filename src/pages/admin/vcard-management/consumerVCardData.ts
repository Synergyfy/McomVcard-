...
export interface ConsumerVCard {
  id: number; preview: string; consumerName: string; memberId: string; membership: string
...
export function consumersForTemplate(templateId: number): ConsumerVCard[] {
  ...
  return ids.map(id => CONSUMER_VCARDS.find(v => v.id === id)).filter((v): v is ConsumerVCard => Boolean(v))
}
