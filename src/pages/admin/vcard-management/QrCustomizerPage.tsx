...
import { DEFAULT_QR_CUSTOMIZATION, getQrCustomization, setQrCustomization, type QrCustomization } from '../../../services/qrCustomizerStore'
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={() => { const el = document.querySelector<HTMLInputElement>('#qr-logo-file'); el?.click() }}
...
export default function QrCustomizerPage() {
  ...
  const initial = getQrCustomization()
  const [cfg, setCfg] = useState<QrCustomization>({
    ...DEFAULT_QR_CUSTOMIZATION,
    qrType: initial.qrType || DEFAULT_QR_CUSTOMIZATION.qrType,
    qrDestination: initial.qrDestination || DEFAULT_QR_CUSTOMIZATION.qrDestination,
    qrDynamic: initial.qrDynamic === '' ? '' : 'true',
    qrColor: initial.qrColor || DEFAULT_QR_CUSTOMIZATION.qrColor,
    qrBgColor: initial.qrBgColor || DEFAULT_QR_CUSTOMIZATION.qrBgColor,
    qrLogo: initial.qrLogo || '',
    qrStyle: initial.qrStyle || DEFAULT_QR_CUSTOMIZATION.qrStyle,
  })
  const [saved, setSaved] = useState(false)
  const [saving, setSaving] = useState(false)
  const [downloading, setDownloading] = useState(false)

  const set = (k: keyof QrCustomization, v: string) => setCfg(prev => ({ ...prev, [k]: v }))

  const handleDownload = async () => {
    if (!cfg.qrDestination.trim()) { toast.error('Enter a destination link'); return }
    setDownloading(true)
    try {
      const url = await qrToPng(cfg.qrDestination || 'https://vcard.mcom/b/this-card', {
        fg: cfg.qrColor,
        bg: cfg.qrBgColor,
        style: cfg.qrStyle,
        logo: cfg.qrLogo,
  ...
  const goBack = () => navigate('/admin/vcard-management/template-builder?tab=content')

  return (
    <div className="space-y-4">
      <Helmet><title>QR Customizer - VCard Management - MCOM VCard</title></Helmet>
      ...
      <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
        ...
        <Link to="/admin/vcard-management/template-builder" className="hover:text-orange-600">Template Builder</Link>
        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" /></svg>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400">Control the link and design the QR — logo, colors and shape.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-xl p-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-emerald-600 dark:text-emerald-400">Your customization is ready to use in the template builder.</p>
            </div>
          </div>
          <button onClick={goBack}
            ...
            Go back to where it is being used in the template builder →
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5 sticky top-4">
            ...
            <div className="rounded-2xl bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 p-8 flex items-center justify-center min-h-[320px]">
              <div className="bg-white dark:bg-gray-900 rounded-xl p-4 shadow-xl">
                <QrCodeSvg value={cfg.qrDestination} fg={cfg.qrColor} bg={cfg.qrBgColor} style={cfg.qrStyle} logo={cfg.qrLogo} sizeClass="w-44 h-44" />
              </div>
            </div>
            <p className="mt-3 flex items-center gap-1.5 text-[9px] text-gray-400">
              ...
              Ready to scan — point a phone camera at the code to open <span className="text-gray-500 truncate">{cfg.qrDestination}</span>
            </p>
            <div className="mt-2 space-y-2 text-[10px]">
              <div className="flex justify-between py-1.5 border-b border-gray-50 dark:border-gray-700">
                ...
                <span className="font-medium text-gray-700 dark:text-gray-200 max-w-[70%] truncate">{cfg.qrType} — {cfg.qrDestination}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-gray-50 dark:border-gray-700">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="text" value={cfg.qrDestination} onChange={e => set('qrDestination', e.target.value)} placeholder="https://…"
                ...
                return (
                  <button key={s.id} onClick={() => set('qrStyle', s.id)}
                    ...
                    <div className="flex justify-center mb-1.5">
                      <QrCodeSvg value={cfg.qrDestination || 'https://vcard.mcom/b/this-card'} fg="#111827" bg="#ffffff" style={s.id} sizeClass="w-10 h-10" />
                    </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4">
            ...
            <input id="qr-logo-file" type="file" accept="image/*" className="hidden"
              onChange={e => {
